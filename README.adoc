...
[source,shell]
----
$ podman image ls | grep -e '-service'
...
WARNING: Ensure the containers from Lab 5 are stopped to free up ports ${INVENTORY_PORT}-${ORDER_PORT}.
...
[source,shell]
----
# 1. Place a test order, targeting port ${ORDER_PORT}
$ curl -X POST \
