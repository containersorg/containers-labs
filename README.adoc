...
NOTE: There is no need to stop any services other than the `order_service` at this point. A new version of the `order_service.py` file has been provided by the engineering team and is already in our `microservices` directory.

=== Updating the Order Service Code

1.  **Go to the terminal (Terminal B)** where the **`order_service`** is currently running (on port ${ORDER_PORT}).
 ...
 * Running on http://127.0.0.1:${ORDER_PORT} (Press CTRL+C to quit)
----
...
[source,shell]
----
# 1. Place a new order using POST method, targeting port ${ORDER_PORT}
$ curl -X POST \
  ...
  "orders": [
    {
      ...
      # ... data confirms we are storing the values in memory ...
    }
  ]
}
----
...
In our monolithic app, this check occurred automatically because all functions were in a single program. Now that they are separate services, how can the order service ask the inventory service about its stock? This is the central challenge—and benefit—of microservices: *inter-service communication*.
****

If we attempt to place an order for more items than are in stock, the order service will still accept it because it is not yet communicating with the inventory service.
...
$ curl -X POST \
    -H "Content-Type: application/json" \
    -d '{"customer_id": "customer_id_1", "medicine": "medicine_A", "quantity": 500}' \
    http://localhost:${ORDER_PORT}/place_order
----

== 2. Enabling Inter-Service Communication

To make our microservices truly functional, the **Order Service** needs to perform an API call to the **Inventory Service (${INVENTORY_PORT})** before finalizing an order.
...
----
$ cd microservices
$ mv order_service.py order_service.py.bak
$ mv ../fix/order_service.py .
----
...
[source,shell]
----
# Attempt to order 150 units of medicine_A (Stock is only 100 on port ${INVENTORY_PORT})
$ curl -X POST \
    -H "Content-Type: application/json" \
...
This fix illustrates that in a microservices architecture, even basic business rules (like stock validation) must be handled by explicit, authenticated **API calls** between the services. This is why services must be network-accessible and independently deployable units—the perfect environment for **containers**.
