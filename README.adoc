...
[source,shell]
----
$ podman run --name inventory_service --network host --env-file ~/.containers-labs.env -dti fedora /bin/bash
$ podman run --name order_service --network host --env-file ~/.containers-labs.env -dti fedora /bin/bash
$ podman run --name customer_service --network host --env-file ~/.containers-labs.env -dti fedora /bin/bash
$ podman run --name billing_service --network host --env-file ~/.containers-labs.env -dti fedora /bin/bash
----
...
[IMPORTANT]
====
A Note on Container Networking: We use the *Host Network Mode* (`--network host`) in these commands. This flag disables network isolation, meaning a service running on port ${INVENTORY_PORT} inside a container is identical to a service running on port ${INVENTORY_PORT} on the host itself. This simplifies cross-container communication for this lab.
====
...
Now, install `Flask` and `requests` libraries on every container, but this time let's use a list in a file to avoid repetition. Create a file named `pharmacy.svc` with the following content:
...
We will test the complex workflow of placing an order, which requires the Order Service (${ORDER_PORT}) to communicate with the Inventory Service (${INVENTORY_PORT}).
...
# 2. Verify the order exists by querying the Order Processing Service (${ORDER_PORT})
