...
Use a free terminal to verify the Inventory Service (Port ${INVENTORY_PORT}) is running:
...
Verify that the Order Service (Port ${ORDER_PORT}) can still communicate with the Inventory Service (Port ${INVENTORY_PORT}) to check stock.
...
# Verify the order exists on port ${ORDER_PORT}
