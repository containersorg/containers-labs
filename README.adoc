...
$ podman run --name inventory_service --network host --env-file ~/.containers-labs.env -d localhost/pharmacy:v3 inventory_service.py
$ podman run --name customer_service --network host --env-file ~/.containers-labs.env -d localhost/pharmacy:v3 customer_service.py
$ podman run --name billing_service --network host --env-file ~/.containers-labs.env -d localhost/pharmacy:v3 billing_service.py
$ podman run --name order_service --network host --env-file ~/.containers-labs.env -d localhost/pharmacy:v3 order_service.py
----
...
The updated `Containerfile` with the fix for version 4 can be found in the solution folder.
...
Modify your local `Containerfile` to match the above or copy it from the solution folder.

[source,shell]
----
# Assuming you are in the microservices directory
$ cp ../solution/Containerfile .
----
...
$ podman rm -f order_service

# Relaunch the order_service container
$ podman run --name order_service --network host --env-file ~/.containers-labs.env -d localhost/pharmacy:v4 order_service.py
---
...
What if something happens to the container? Let's stop --> remove --> restart --> check if the data is still there.
...
The data is currently saved on the container's writable layer. If we stop and remove the container, we lose the data. To achieve true persistence across container restarts, we must convert the `/data` folder into a **Volume**.
...
Verify the volume was created and take note of its mountpoint:
...
$ podman stop order_service
$ podman rm order_service
$ podman run --rm -d --name order_service --network host \
    -v pharmacy_orders_data:/data:rw \
    --env-file ~/.containers-labs.env \
    localhost/pharmacy:v4 order_service.py
----
...
[
  ...
  # 4. Using the noted mountpoint path check the file contents directly from the host system. (Replace the username with your actual username)

$ cat /home/username/.local/share/containers/storage/volumes/pharmacy_orders_data/_data/orders.json
[
  {
    "customer_id": "customer_id_1",
    # ... first order data ...
