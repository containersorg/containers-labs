...
[NOTE]
====
. We added the (`--rm`) option to ensure the containers are deleted from our system once they are stopped.
...
Update pip on every container:

[source,shell]
----
$ podman exec -ti inventory_service pip install --upgrade pip
$ podman exec -ti order_service pip install --upgrade pip
$ podman exec -ti customer_service pip install --upgrade pip
$ podman exec -ti billing_service pip install --upgrade pip
----

Now, install `Flask` and `requests` libraries on every container:
...
To verify that our files were successfully copied:

[source,shell]
----
$ podman exec -ti inventory_service ls -l *_service.py
# ... and repeat for the other three containers ...
----
...
WARNING: Since we used the `--rm` flag when running the containers, simply stopping them will delete them permanently.
...
# Verify they are gone (podman ps should be empty)
$ podman ps -a
----
