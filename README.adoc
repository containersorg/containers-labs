...
Ensure all previous containers are stopped and removed before starting new ones.

[source,shell]
----
$ podman rm -f inventory_service order_service customer_service billing_service
----

[source,shell]
----
podman run --name inventory_service --network host --env-file ~/.containers-labs.env -d localhost/pharmacy:v2 inventory_service.py
podman run --name order_service --network host --env-file ~/.containers-labs.env -d localhost/pharmacy:v2 order_service.py
podman run --name customer_service --network host --env-file ~/.containers-labs.env -d localhost/pharmacy:v2 customer_service.py
podman run --name billing_service --network host --env-file ~/.containers-labs.env -d localhost/pharmacy:v2 billing_service.py
----
...
Test the Inventory Service (Port ${INVENTORY_PORT}) using the loopback address (`0.0.0.0` or `127.0.0.1`):
...
[source,shell]
----
# Place a test order, targeting port ${ORDER_PORT}
$ curl -X POST \
