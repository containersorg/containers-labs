...
We install K3s as a system service.
...
The k3s server can be started and stopped with the following commands:
...
== 2. Defining the Application Deployment (YAML)

We must define two things for our application: the **Deployment** (which manages the running pods and guarantees replicas) and the **Service** (which provides a stable network endpoint for the pods).

// # ... Repeat Deployment and Service for inventory-service (Port 5001)
// # ... Repeat Deployment and Service for customer-service (Port 5003)
// # ... Repeat Deployment and Service for order-service (Port 5002, with Volume claim)
...
deployment.apps/billing-deployment created
----
...
💡 TIP: The `ImagePullBackOff` error occurs because K3s (using ContainerD) cannot access the local Podman image repository (`localhost/pharmacy:latest`). It needs the image loaded into its own internal registry.
...
----
$ sudo kubectl apply -f deployment.yaml
deployment.apps/billing-deployment configured

$ sudo kubectl apply -f service.yaml
service/billing-service configured
----
