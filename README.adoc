...
[%header,cols="2,3,^1,1",align="center",stripes="even",width="100%"]
|===
^| Service              ^| Responsibility                                                   ^| Port     ^| File
| Inventory Management  | Handles all operations related to inventory stock and pricing.    | `${INVENTORY_PORT}`    | `inventory_service.py`
| Order Processing      | Oversees order placement and tracking.                            | `${ORDER_PORT}`    | `order_service.py`
| Customer Management   | Dedicated to handling customer information and details.           | `${CUSTOMER_PORT}`    | `customer_service.py`
| Billing Service       | Responsible for managing billing and payment processes.           | `${BILLING_PORT}`    | `billing_service.py`
|===
...
Change to the microservices directory and use `grep` to confirm that each file is configured to run on its designated port:
...
[source,shell]
----
# TEST 1: Requesting Inventory data on the correct port ${INVENTORY_PORT}
$ curl http://127.0.0.1:${INVENTORY_PORT}/view_inventory
{"inventory":{...}} 
# Output: Success. Inventory data is returned.

# TEST 2: Requesting Inventory data on the Order Service port {ORDER_PORT} (incorrect)
$ curl http://127.0.0.1:${ORDER_PORT}/view_inventory
<!doctype html>
<html lang=en>
...
We can access the Customer Management Service directly on port ${CUSTOMER_PORT}.

[source,shell]
----
$ curl http://127.0.0.1:${CUSTOMER_PORT}/view_customer/customer_id_1
{
  "customer_details": {
    "address": "123 Main St",
    "name": "John Doe"
...
Order placement is managed by the Order Processing Service on port ${ORDER_PORT}. This service is responsible for adding the transaction data.

[source,shell]
----
# Place a new order using POST method, targeting port ${ORDER_PORT}
$ curl -X POST \
...
Verify the order exists by querying the Order Processing Service on port ${ORDER_PORT} again:
...
In this microservices model, if the **Inventory Management Service {INVENTORY_PORT}** fails, the **Order Processing Service {ORDER_PORT}** and the **Customer Management Service {CUSTOMER_PORT}** would remain running, allowing partial functionality to continue.
