...
$ cd monolithic
$ python3 pharmacy.py
 * Running on http://127.0.0.1:${MONO_PORT} (Press CTRL+C to quit)
----
...
The Flask web application is now operational and accessible via the loopback address (`127.0.0.1`) on port ${MONO_PORT}. We will use `CURL` to verify the application endpoints.
...
$ curl http://127.0.0.1:${MONO_PORT}/view_inventory

# Retrieve John Doe's customer details
$ curl http://127.0.0.1:${MONO_PORT}/view_customer/customer_id_1
----
...
WARNING: Before continuing to the next lab, you **must** terminate the monolithic application running on port ${MONO_PORT}.
