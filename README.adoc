...
Ensure you have either `podman-compose` or `docker-compose` installed and the final, unified image from Lab 9 is ready.
...
Compose manages the creation of volumes, networks, and containers automatically based on the `compose.yaml` file.
...
NOTE: Notice that new networks, volumes, and containers were created, all typically prepended with the name of our project directory (e.g., `lab12_pharmacy_app_network`).
